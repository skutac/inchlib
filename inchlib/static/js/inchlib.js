...
function InCHlib(element_id, user_settings){
    ...
    this.settings.width = (user_settings.max_width && user_settings.max_width < element_width)?user_settings.max_width:this.settings.width;
...
InCHlib.prototype.add_prefix = function(){
    ...
    for(i in this.data.nodes){
        id = [this.settings.element_id, i].join("#");
        this.data.nodes[id] = this.data.nodes[i];
        delete this.data.nodes[i];
        if("parent" in this.data.nodes[id]){
            this.data.nodes[id].parent = [this.settings.element_id, this.data.nodes[id].parent].join("#");
        }
        if(this.data.nodes[id]["count"] != 1){
            this.data.nodes[id].left_id = [this.settings.element_id, this.data.nodes[id].left_id].join("#");
            this.data.nodes[id].right_id = [this.settings.element_id, this.data.nodes[id].right_id].join("#");
        }
    }
...
InCHlib.prototype.draw_navigation = function(){
    ...
    if(self.zoomed_clusters.length>0){
        ...
        var back_icon = new Kinetic.Path({
            data: "M12.981,9.073V6.817l-12.106,6.99l12.106,6.99v-2.422c3.285-0.002,9.052,0.28,9.052,2.269c0,2.78-6.023,4.263-6.023,4.263v2.132c0,0,13.53,0.463,13.53-9.823C29.54,9.134,17.952,8.831,12.981,9.073z",
            x: 90,
            y: 10,
            fill: "grey",
            scale: 1,
        ...
        back_overlay.on("mouseover", function(){
            self.icon_mouseover(back_icon, self.navigation_layer);
        });

        back_overlay.on("mouseout", function(){
            self.icon_mouseout(back_icon, self.navigation_layer);
        });
        ...
        refresh_overlay.on("mouseover", function(){
            self.icon_mouseover(refresh_icon, self.navigation_layer);
        });

        refresh_overlay.on("mouseout", function(){
            self.icon_mouseout(refresh_icon, self.navigation_layer);
        });
    }
    ...
    if(self.header){
        ...
        filter_overlay.on("mouseover", function(){
            self.icon_mouseover(filter_icon, self.navigation_layer);
        });
           
        filter_overlay.on("mouseout", function(){
            self.icon_mouseout(filter_icon, self.navigation_layer);
        });
    }
...
InCHlib.prototype.highlight_path = function(path_id){
 ...
 InCHlib.prototype.highlight_rows = function(row_ids, action){
    ...
    for(i = 0; i<row_ids.length; i++){
        prefixed_row_ids.push([this.settings.element_id, row_ids[i]].join("#"));
    }
...
InCHlib.prototype.draw_cluster_layer = function(){
    ...
    zoom_overlay.on("mouseover", function(){
        self.icon_mouseover(zoom_icon, self.cluster_layer);
    });

    zoom_overlay.on("mouseout", function(){
        self.icon_mouseout(zoom_icon, self.cluster_layer);
    });
...
InCHlib.prototype.filter_icon_click = function(filter_button){
    ...
    var position = $("#" + this.settings.element_id).position();
    ...
    if(filter_list.length > 0){
        ...
        $("#" + self.settings.element_id).append("<div id='dendrogram_filter_features'><ul>" + filter_list + "</ul><hr /><div><span id='cancel_filter_list'>Cancel</span>&nbsp;&nbsp;&nbsp;<span id='update_filter_list'>Update</span></div></div>");
        ...
        $("#update_filter_list").click(function(){
            ...
            if(self.visible_features_equal_column_dendrogram_count()){
...
InCHlib.prototype.icon_mouseover = function(icon, layer){
    icon.setFill("black");
    layer.draw();
}

InCHlib.prototype.icon_mouseout = function(icon, layer){
    icon.setFill("grey");
    layer.draw();
}
...
InCHlib.prototype.dendrogram_layers_mouseout = function(evt){
    ...
    if(shape_type == "Rect"){
        var overlay = this.dendrogram_hover_layer.get("#" + [this.settings.element_id, "path_hover"].join("_"))[0];
        if(overlay){
            this.dendrogram_hover_layer.get("#" + [this.settings.element_id, "path_hover"].join("_"))[0].destroy();
...
InCHlib.prototype.dendrogram_layers_mouseover = function(evt){
    ...
    if(shape_type == "Rect"){
        ...
        path_overlay.setId([this.settings.element_id, "path_hover"].join("_"))
